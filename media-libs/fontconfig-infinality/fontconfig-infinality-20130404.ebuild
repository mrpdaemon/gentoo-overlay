...
RDEPEND="app-admin/eselect-fontconfig
	...
	bohoomil? ( media-fonts/croscorefonts )
...
src_prepare() {
	if use bohoomil; then
		mkdir "${WORKDIR}"/bohoomil
		cp "${FILESDIR}"/bohoomil/conf.d/* "${WORKDIR}"/bohoomil/

		epatch "${FILESDIR}"/bohoomil/72-dpi.patch
	fi
}
